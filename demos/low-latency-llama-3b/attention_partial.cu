...
template <typename config, typename globals> struct attention_partial {
    ...
    // KV tiles doubled to 16×128 = 4096 bytes each.
    // NUM_STAGES=2: 2×2×4096 = 16384 bytes → fits one 16KB page.
    static constexpr int NUM_STAGES = 2;
    static constexpr int GQA_RATIO =
        LLAMA_3B_NUM_ATTENTION_HEADS / LLAMA_3B_NUM_KV_HEADS;  // 3
    static constexpr int QOL_PAGE = 0;
    static constexpr int KV_PAGE = 1;

    static_assert(GQA_RATIO == 3, "GQA_RATIO must be 3 for 3B.");
    static_assert(NUM_STAGES <= 2, "Modify page allocation for KVs.");

    using q_rt = kittens::rt_bf<16, LLAMA_3B_HEAD_DIM>;  // 16×128, only 3 rows used
    using q_st = kittens::st_bf<16, LLAMA_3B_HEAD_DIM>;  // 16×128, only 3 rows used
    ...
    using attn_fl_rt = kittens::rt_fl<16, LLAMA_3B_KV_BLOCK_SIZE>;  // only 3 values used
    using attn_bf_rt = kittens::rt_bf<16, LLAMA_3B_KV_BLOCK_SIZE>;
    using max_vec_rv = col_vec<kittens::rt_fl<16, LLAMA_3B_HEAD_DIM>>;
    using max_vec_sv = kittens::sv_fl<16>;
    using norm_vec_rv = col_vec<kittens::rt_fl<16, LLAMA_3B_HEAD_DIM>>;
    using norm_vec_sv = kittens::sv_fl<16>;
    using l_rv = col_vec<kittens::rt_fl<16, LLAMA_3B_HEAD_DIM>>;
    using l_sv = kittens::sv_fl<16>;
    using o_rt = kittens::rt_fl<16, LLAMA_3B_HEAD_DIM>;  // 16×128
    using o_sv = kittens::sv_fl<LLAMA_3B_HEAD_DIM>;       // 128 floats = 512 bytes
    using o_sv_bf = kittens::sv_bf<LLAMA_3B_HEAD_DIM>;    // 128 bf16 = 256 bytes
    ...
    // QOL page layout for head_dim=128, GQA=3:
    //   q_st: st_bf<16,128> = 4096 bytes
    //   o_sv[3]: 3 × sv_fl<128> = 3 × 512 = 1536 bytes
    //   l_sv: sv_fl<16> = 64 bytes
    //   Total: ~5696 bytes, fits in one 16KB page
    ...
    __device__ static inline o_sv (&get_O_smem(megakernel::state<config> &s))[3] {
        int pid = s.pid(QOL_PAGE);
        return *reinterpret_cast<o_sv(*)[3]>(
    ...
    //   Each kv_st = st_bf<16,128> = 4096 bytes
    //   2 stages × 2 × 4096 = 16384 bytes = exactly one page
    ...
    // Store 3 rows from a 16-row register tile into 3 separate sv_fl<128> vectors.
    // Each Q head occupies rows: head0=rows 0-3, head1=rows 4-7, head2=rows 8-11.
    // But for GQA=3, the 3 heads are stored at row offsets determined by
    // q_head_start_idx % 16.
    template <ducks::sv::all SV, ducks::rt::all RT>
    __device__ static inline void
    store_3_rows(SV (&dst)[3], const RT &src, int row3idx /*= 0, 1, or 2*/) {
        ...
        // For 3 heads at row positions determined by row3idx:
        // row3idx=0: rows 0-3 (even half of first pack, laneid < 16)
        // row3idx=1: rows 4-7 (even half of second pack, laneid >= 16)
        // row3idx=2: rows 8-11 (odd half of first pack, laneid < 16)
        // This matches the MMA register layout where:
        //   data[0] = rows 0-3 (laneid 0-15), data[1] = rows 8-11 (laneid 0-15)
        //   data[2] = rows 0-3 (laneid 16-31), data[3] = rows 8-11 (laneid 16-31)
        //   Wait, that's for row_l. Let me use the same approach as 1B.

        // Guard: only 3 outputs, skip local_row_idx == 3
...
#pragma unroll
    ...
    // Load Q for 3B: head_dim=128, GQA_RATIO=3
    // Load 3 heads × 128 elements = 384 bf16 = 768 bytes
    // A warp has 32 threads; each can do one 16-byte (float4) copy.
    // Per round: 32 × 8 = 256 bf16 elements. Need 384/256 ≈ 1.5 rounds → 2 rounds.
    __device__ static inline void
    load_Q_async(q_st &dst, const globals::activations_t &src,
                 const int q_head_start_idx /*0, 3, 6, ...*/) {
        using T = typename q_st::dtype;
        constexpr int elem_per_memcpy = sizeof(float4) / sizeof(T);  // 8
        constexpr int total_elements = GQA_RATIO * LLAMA_3B_HEAD_DIM;  // 3×128=384
        constexpr int total_memcpys = (total_elements + elem_per_memcpy - 1) / elem_per_memcpy;  // 48
        ...
        // overflowing the 16-row tile (GQA=3 doesn't divide 16).
        uint32_t dst_base = static_cast<uint32_t>(__cvta_generic_to_shared(
            &dst.data[0]));

        int laneid = kittens::warp::laneid();

        // Round 1: first 32 copies (elements 0..255)
        if (laneid < total_memcpys) {
            int flat_elem = laneid * elem_per_memcpy;
            int row = flat_elem / LLAMA_3B_HEAD_DIM;
            int col = flat_elem % LLAMA_3B_HEAD_DIM;
        ...
        // Round 2: remaining copies (elements 256..383)
    ...
    struct launcher {
        static __device__ void wait_for_kv(const globals &g, megakernel::state<config> &s,
            ...
            // head_dim=128, matvec_block_size=16 → 8 blocks per head
            // Wait for K head blocks
            while (*(volatile int *)&g.Bar[{
                       inst.layer_idx, OPCODE_RMS_QKV_MatVecRopeAppend - 1,
                       LLAMA_3B_NUM_ATTENTION_HEADS + inst.kv_head_idx}] < 8) {
                ...
                *(volatile int *)&g
                     .Bar[{inst.layer_idx, OPCODE_RMS_QKV_MatVecRopeAppend - 1,
                           LLAMA_3B_NUM_ATTENTION_HEADS +
                               LLAMA_3B_NUM_KV_HEADS + inst.kv_head_idx}] < 8) {
                __nanosleep(config::GMEM_SPIN_LOOP_SLEEP_NANOS);
            }
    ...
    struct consumer {
        static __device__ void run(const globals &g, megakernel::state<config> &s) {
            ...
            if (kittens::warpid() == 0) {
                ...
                if (kittens::laneid() == 0) {
                    // head_dim=128, block_size=16 → 8 blocks per head
                ...
                for (int i = 0; i + start_blk_idx < end_blk_idx; ++i) {
                    ...
                    if ((i + start_blk_idx + 1) * LLAMA_3B_KV_BLOCK_SIZE > seq_len)
                ...
                // Store: 3 heads
                store_3_rows(O_smem, O_reg, q_head_local_idx);
                kittens::warp::sync();
