...
template <typename Config, typename Globals> struct rms_qkv_rope_append {
    static constexpr int opcode = OPCODE_RMS_QKV_MatVecRopeAppend;

    // 3B QKV layout: Q=24×128=3072, K=8×128=1024, V=8×128=1024 → total=5120
    // Block boundaries in units of matvec_block_size=16:
    static constexpr int K_BLK_START =
        Globals::num_attention_heads * Globals::head_dim /
        Globals::matvec_block_size;  // 24*128/16 = 192
    static constexpr int V_BLK_START =
        (Globals::num_attention_heads + Globals::num_kv_heads) *
        Globals::head_dim / Globals::matvec_block_size;  // 32*128/16 = 256

    // Barrier: downproj of previous layer.
    // With matvec_reduction_size=1024: (8192/1024)×(3072/16) = 8×192 = 1536
    static constexpr int EXPECTED_ARRIVAL_COUNT =
        (Globals::intermediate_dim / 1024) *
        (Globals::hidden_dim / Globals::matvec_block_size);  // 1536

    // RoPE scratch: head_dim=128 → 128 floats cos + 128 floats sin = 1024 bytes
    ...
    struct pipeline_specifics {
        ...
        store(megakernel::state<Config> &s, const Globals &g,
            ...
            // Each sv_fl<16> = 64 bytes, so offset = head_chunk * 64
            ...
            if (block_idx < V_BLK_START) { // only Q & K need RoPE
                ...
                // Fetch the neighbor values for interleaved RoPE
            ...
            if (kittens::laneid() == 0) {
                ...
                // barrier: blocks per head = head_dim/16 = 8
                // So block_idx / 8 gives the head index for barrier signaling
                atomicAdd(
