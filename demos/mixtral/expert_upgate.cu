...
template <typename Config, typename Globals> struct expert_upgate {
    ...
    struct pipeline_specifics {
        ...
        static __device__ inline void store(megakernel::state<Config> &s,
            ...
            // recomputing the gate+up matvec result directly from global memory.
            if constexpr (pipeline::REDUCTION_DIM_PER_WARP < 64) {
                if (kittens::laneid() < 16) {
                    ...
                    if (lane == 0 && true_output_idx == 0 && inst.expert_idx < 4) {
                        printf("DBG upgate SM=%d exp=%d blk=%d is_active=%d act0=%f up_acc=%f result=%f\n",
                            (int)blockIdx.x, inst.expert_idx, block_idx,
                            (int)is_active,
                            float(__ldcg(&g.router_normed_hidden.raw_ptr[0])),
                            up_acc, result);
                    }
    ...
    struct consumer {
        static __device__ void run(const Globals &g,
            ...
            // consumer_loop handles all semaphore handshaking with the loader/storer.
            // The bypass for RDPW<64 (small test) is handled inside store(), which
            // recomputes the result from global memory instead of using the garbage
            // partial sums left by matvec() with a mismatched swizzle pattern.
    ...
    struct storer {
        static __device__ void run(const Globals &g,
            ...
            if (kittens::laneid() == 0) {
                ...
                // Wait for all TMA stores to complete before signaling the barrier.
                // This ensures expert_silu_out is globally visible before downstream
                // expert_downproj SMs read it.
