...
import pydra
import torch
from bokeh.models import (  # Changed Segment to Rect  # Changed Segment to Rect
    ColumnDataSource,
    HoverTool,
    Legend,
    LegendItem,
...
from bokeh.palettes import (  # Using Category10 for distinct instruction types  # Using Category10 for distinct instruction types
    Category10,
    Viridis256,
)
...
MAPPING = {
    ...
    7: ("launcher_start", START_SYMBOL, "blue"),
    8: ("launcher_end", END_SYMBOL, "blue"),
    9: ("storer_start", START_SYMBOL, "red"),
    10: ("storer_end", END_SYMBOL, "red"),
    11: ("consumer_start", START_SYMBOL, "purple"),
    12: ("consumer_end", END_SYMBOL, "purple"),
    44: ("at_gmem_wait", START_SYMBOL, "yellow"),
    45: ("done_gmem_wait", END_SYMBOL, "yellow"),
    46: ("at_gmem_store", START_SYMBOL, PINK),
    47: ("done_gmem_store", END_SYMBOL, PINK),
    48: ("first_load", START_SYMBOL, CYAN),
    49: ("first_use", START_SYMBOL, "white"),
    50: ("first_store", START_SYMBOL, "black"),
    51: ("last_load", END_SYMBOL, CYAN),
    52: ("last_use", END_SYMBOL, "white"),
    53: ("last_store", END_SYMBOL, "black"),
}

for key, (name, symbol, color) in MAPPING.items():
    EVENT_CATEGORIES[key] = name
    EVENT_DESCRIPTIONS[key] = name
    MARKER_STYLES[name] = {"marker": symbol, "color": color, "size": 8, "legend": name}


def save_gantt_chart_bokeh(
    Timings,
    Instructions,
    k_slots,
    save_all=False,
    ...
    if Timings.is_cuda:
        Timings = Timings.cpu()
    if Instructions.is_cuda:
        Instructions = Instructions.cpu()
    ...
    marker_data = {
        category: {"x": [], "y": [], "name": [], "proc": [], "instr_idx": []}
    ...
    print(
        f"Processing data for {num_processors} processors, {num_instructions} instructions..."
    )
    for proc in range(num_processors):
        for instr in range(num_instructions):
            ...
            if start > 0 and end > start:
                ...
                y_base = proc
                bottom = (
                    y_base - 0.5 + (slot_idx * bar_height) + (slot_idx + 1) * bar_margin
                )
                ...
                if save_all:
                    for event_id in range(128):
                        ...
                        if event_time > 0 and event_time >= start and event_time <= end:
                            ...
                            if category:
                                event_name = EVENT_DESCRIPTIONS.get(
    ...
    plot_width = max(
        800, min(6000, int(plot_height * max(5, time_diff / (num_processors * 5))))
    )
    ...
    p = figure(
        ...
        x_range=(
            min_time_overall - 0.05 * time_diff,
            max_time_overall + 0.05 * time_diff,
        ),
        title="Mixtral Instruction Execution Timeline (Interactive)",
    ...
    for proc_idx in range(num_processors):
        bg_colors.append(
            BACKGROUND_COLOR_EVEN if proc_idx % 2 == 0 else BACKGROUND_COLOR_ODD
        )
    ...
    if bar_data["left"]:
        ...
        for instr_type_code in sorted_types:
            if instr_type_code not in seen_types:
                instr_name = INSTRUCTION_MAP.get(
                    instr_type_code, f"Unknown ({instr_type_code})"
                )
                color = COLOR_MAP.get(instr_type_code, "gray")
                dummy_renderer = p.rect(
                    x=0,
                    y=0,
                    width=0,
                    height=0,
                ...
                instr_legend_items.append(
                    LegendItem(
                        label=f"{instr_name} ({instr_type_code})",
    ...
    if save_all:
        sorted_categories = sorted(
            marker_renderers.keys(),
            key=lambda cat: list(MARKER_STYLES.keys()).index(cat),
        )
        for category in sorted_categories:
            renderer = marker_renderers.get(category)
            if renderer:
                marker_legend_items.append(
                    LegendItem(
                        label=MARKER_STYLES[category]["legend"], renderers=[renderer]
                    )
                )
    ...
    save(
        p,
        filename=filename,
        title=f"Mixtral Instruction Timeline {name if name else ''}",
        resources=CDN,
    )
    ...
    if verbose:
        if len(Timings.shape) == 3 and Timings.shape[2] >= 128:
            valid_mask = (Timings[:, :, 0] > 0) & (
                Timings[:, :, 127] > Timings[:, :, 0]
            )
            if valid_mask.any():
                ...
                print(
                    f"Total execution time (sum of durations): {valid_durations_us.sum():.3f}"
                )
                print(
                    f"Overall start time (min start time): {valid_start_us.min():.3f}"
                )
                ...
                if len(Instructions.shape) >= 2:
                    ...
                    if (
                        valid_instructions.numel() > 0
                        and valid_instructions.shape[-1] > 0
                    ):
                        ...
                        for instr_type_code_tensor in sorted(unique_types):
                            ...
                            type_mask_on_valid = (
                                valid_instructions[:, 0] == instr_type_code
                            )
                            if type_mask_on_valid.any():
                                mean_time = valid_durations_us[
                                    type_mask_on_valid
                                ].mean()
                                count = type_mask_on_valid.sum().item()
                                type_name = INSTRUCTION_MAP.get(
                                    instr_type_code, f"Unknown ({instr_type_code})"
                                )
                                print(
                                    f"  {type_name} ({instr_type_code}): {mean_time:.3f} us (Count: {count})"
                                )
                    else:
                        print(
                            "  Could not extract instruction types from valid instructions."
                        )
                else:
                    print(
                        "  Instructions tensor shape is not as expected for statistics."
                    )
            else:
