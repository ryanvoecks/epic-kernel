"""
Python reference VM for Llama 3.2 3B latency decode.

Structurally identical to the 1B VM (megakernels.demos.latency.python_vm)
but all hard-coded barrier counts are replaced with expressions derived from
the Globals dimensions so they are correct for 3B:

  hidden_size       = 3072   (was 2048)
  intermediate_size = 8192   (same, but 8192/3072 is not integer)
  head_dim          = 128    (was 64)
  num_hidden_layers = 28     (was 16)
  matvec_reduction_size = 1024  (divides 8192 evenly → 8 col-splits)
"""
...
# ── derived barrier counts ──────────────────────────────────────────────────


def _blocks_per_head(globs: Globals) -> int:
    """Number of QKV blocks that span one head (head_dim / qkv_block_size)."""
    return globs.head_dim // globs.qkv_block_size


def _expected_downproj_barrier(globs: Globals) -> int:
    """Total barrier increments produced by all DownProjResidual instructions
    in a single layer = num_col_splits × num_down_blocks."""
...
def _expected_upgate_barrier(globs: Globals) -> int:
    """Total upgate completions per layer = intermediate_size / block_size."""
    return globs.intermediate_size // globs.up_gate_proj_block_size


def _expected_oproj_barrier(globs: Globals) -> int:
    """Total o-proj completions per layer = hidden_size / block_size."""
...
def o_proj_residual(globals: Globals, instruction: O_ProjResidual):
    # Barrier check: all attention heads must have completed reduction
...
def down_proj_residual(globals: Globals, instruction: DownProjResidual):
    # Barrier check: all upgate blocks for this layer must be done
    op_barriers = globals.barriers[instruction.layer_idx, instruction.prev_opcode() - 1]
...
def layer_norm_double_matvec_silu(
    ...
    # Barrier check: all o-proj blocks for this layer must be done
    op_barriers = globals.barriers[instruction.layer_idx, instruction.prev_opcode() - 1]
...
def layer_norm_matvec_rope_append(
    ...
    # Barrier check: previous layer's downproj must be fully complete
    if layer_idx > 0:
...
def rms_lm_head(globals: Globals, instruction: RMS_LM_Head):
    # Barrier check: last layer's downproj must be fully complete
...
def partial_attention(globals: Globals, instruction: PartialAttention):
    ...
    # Barrier check: Q/K/V heads for this kv_head must be ready
    op_barriers = globals.barriers[instruction.layer_idx, instruction.prev_opcode() - 1]
